...
- installed AgentFlow `0.1.0` under `%USERPROFILE%\.agentflow` (read-only)
...
| parallel spawn/fan-out | specialist/background workers | graph fan-out | conditional | AgentFlow when safe or no cull is needed; guarded direct culling otherwise |
