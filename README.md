...
- AgentFlow Pack mode can use native fanout-member cancellation through a read-only periodic controller when platform, policy, and budget permit. It is disabled on Windows because AgentFlow's local cancellation bypasses Hound's guarded PID-tree checks; use `--pack-backend direct` for safe Windows live culling. Hound never discovers an inner PID or uses whole-run cancellation for a per-member cull.
