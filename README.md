...
Hound keeps one Codex Writer working until machine checks and an independent Verifier accept the result. When uncertainty, repeated failure, or explicit Pack policy justifies extra investigation, Hound runs bounded read-only Scouts and returns their evidence to the single Writer. A qualified `auto` Pack prefers installed AgentFlow when native member culling is safe or unnecessary; otherwise it selects the guarded direct supervisor.
...
Pack backend defaults to adaptive `auto`. Whenever a Pack has at least two distinct missions, at least two workers fit concurrency and budget, and the enabled AgentFlow CLI is available, Hound delegates the bounded read-only hunt unless doing so would lose required live member culling. On Windows, a Pack with more Scouts than configured survivors therefore routes directly through Hound; AgentFlow remains eligible when every Scout is retained. This includes default missions derived from blocked, idle, or repeated-failure evidence. Missing or failed AgentFlow also falls back to direct supervision. Override explicitly with `--pack-backend agentflow` or `--pack-backend direct`.

Each round returns `continue`, `candidate_done`, or `blocked`; only Hound can set the durable run status. A scheduled action whose fingerprint already failed with the same evidence is refused before another Writer starts; Hound seeks different Scout evidence or stops honestly. The Writer does not duplicate the contract's authoritative machine commands: `candidate_done` hands the candidate to Hound for those gates and does not claim they already passed. Completion still requires machine gates, baseline-relative path gates, and final review. The first Verifier attempt uses `rounds/<n>/verifier/`; attempts made while resuming the same round use `rounds/<n>/review-resume-<k>/verifier/`. `final/verification.json` and `final/verifier.json` snapshot the latest completion evidence before terminal `state.json` becomes the commit marker. After the same final-review failure repeats, Hound runs a Pack when policy and budget allow; if that Pack adds no evidence, the run stops `BLOCKED` instead of repeating another empty review cycle. Unexpected engine defects become durable `failed_internal` runs.
...
Scouts are read-only, bounded by `concurrency`, and receive distinct missions. A qualified `auto` request that does not require guarded direct culling writes a small AgentFlow JSON pipeline with one Scout fanout group and one bounded retry per Scout. On platforms where native member cancellation preserves Hound's process-safety contract, semantic judging, budget, and survivor headroom may add one read-only periodic culling controller. The controller is told to cancel at most one redundant or clearly lagging member per interval, never on its first tick, and to preserve the survivor floor and unique evidence. Hound audits AgentFlow's native action artifacts and applied-action log, rejects reruns, multi-target actions, outside-fanout targets, and survivor-floor breaches, and imports only confirmed applied member cancellations as `culled`. Windows deliberately omits this controller because AgentFlow's local runner does not use Hound's guarded PID-tree termination; `auto` sends cullable Packs to the safe direct backend instead. Hound then ranks the evidence and marks the configured survivor set for synthesis. A native node counts as completed evidence only when its final response matches Hound's Scout schema; JSONL progress events cannot masquerade as a final result. If Hound's outer timeout ends the AgentFlow process tree, or Hound itself later resumes after a crash, completed and confirmed-culled AgentFlow nodes are recovered from durable native `run.json` and event data; other unfinished nodes become `interrupted`.
...
- AgentFlow Pack mode can use native fanout-member cancellation through a read-only periodic controller when platform, policy, and budget permit. It is disabled on Windows because AgentFlow's local cancellation bypasses Hound's guarded PID-tree checks; `auto` routes cullable Windows Packs to the direct backend. Hound never discovers an inner PID or uses whole-run cancellation for a per-member cull.
