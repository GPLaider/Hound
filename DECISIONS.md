...
12. AgentFlow mode uses whole-pack timeout and post-run ranking. Direct mode retains live per-Scout culling because AgentFlow exposes safe whole-run, not per-node, CLI cancellation.
...
24. AgentFlow nodes receive one bounded retry, but Hound performs only honest post-run ranking and whole-run cancellation; live per-node quality culling remains direct-only.
